...
#******************************************************************************#
#* Data cleaning
all_rate_parameters_source_destination_coords <-  vector("list", 3)
...
for (i in c(1:3)){
  ...
  rate_parameters_source_destination_coords$Serotype = i
  
  #append the final ones to the list. 
  all_rate_parameters_source_destination_coords[[i]] <- rate_parameters_source_destination_coords
}
#******************************************************************************#

#******************************************************************************#
#* Combine the files for the three serotypes. 
rate_parameters_source_destination_coords <- rbind(all_rate_parameters_source_destination_coords[[1]], 
                                                   all_rate_parameters_source_destination_coords[[2]], 
                                                   all_rate_parameters_source_destination_coords[[3]])
...
rate_parameters_evidence_exportations$Serotype <- as.factor(rate_parameters_evidence_exportations$Serotype)
rate_parameters_evidence_importations$Serotype <- as.factor(rate_parameters_evidence_importations$Serotype)
...
global_transmission_network_exportations_importations_plot <- ggplot() +
  ...
  ##Exportations
  geom_curve(data=rate_parameters_evidence_exportations, aes(x = Long.x, y = Lat.x, xend = Long.y, yend = Lat.y, size = Bayes, colour = Serotype), curvature = 0.3, 
  ...
  ##Importations
  geom_curve(data=rate_parameters_evidence_importations, aes(x = Long.x, y = Lat.x, xend = Long.y, yend = Lat.y, size = Bayes, colour = Serotype), curvature = 0.3, 
             arrow = arrow(angle = 20, length = unit(0.2, "inches"),
                           ends = "last", type = "closed"), linetype = 1) + 
  geom_text(data=rate_parameters_evidence_importations, aes(label = Label.x, x = Long.x, y = Lat.x),
            nudge_y = rate_parameters_evidence_importations$Nudge_lat, nudge_x = rate_parameters_evidence_importations$Nudge_long, size = 6) + 
  ...
  # Size scale
  scale_size_manual(name = "Support \n(Bayes Factors)", values = c('1' = 0.4, '2' = 1.2, '3' = 2.2), labels = c("Moderate", "Strong", "Very Strong"), guide = "legend", drop = FALSE) +
  scale_colour_manual(name = "DENV Serotype", values = c('1' = 'firebrick1', '2' ='green4', '3' = 'dodgerblue'), labels = c("1", "3", "4"), guide = "legend", drop = FALSE) +
...
#*
#*Global gene flow
rate_parameters_evidence_global$Serotype <- as.factor(rate_parameters_evidence_global$Serotype)
...
global_transmission_global <- ggplot() +
  ...
  # Add other regions
  geom_point(data = rate_parameters_evidence_global, aes(x = Long.y, y = Lat.y), size = 5, color = "grey40", shape = 16) + 
  
  #Add connection lines
  # Will add median and support and legend. 
  geom_curve(data=rate_parameters_evidence_global, aes(x = Long.x, y = Lat.x, xend = Long.y, yend = Lat.y, size = Bayes, colour = Serotype), curvature = 0.3, 
             arrow = arrow(angle = 15, length = unit(0.2, "inches"),
                           ends = "last", type = "closed"), linetype = 1) + 
